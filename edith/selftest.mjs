/**
 * ULTRON self-test — exercises the REAL tool layer against a throwaway workspace.
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "ultron-selftest-"));
...
console.log(`ULTRON self-test in ${tmp}\n`);

// 1. write + read
let r = await tools["ultron.write_file"].run({ file: "src/index.js", content: "console.log('hi from ultron');\n" });
assert.equal(r.action, "created"); ok("write_file creates a real file");
r = await tools["ultron.read_file"].run({ file: "src/index.js" });
assert.match(r.content, /hi from ultron/); ok("read_file returns real contents");

// 2. edit
r = await tools["ultron.edit_file"].run({ file: "src/index.js", find: "hi from ultron", replace: "hello world" });
...
// 3. search
r = await tools["ultron.search_code"].run({ query: "hello world" });
assert.ok(r.matches.length >= 1); ok("search_code finds real matches");

// 4. run_command (real stdout + exit code)
r = await tools["ultron.run_command"].run({ command: "node src/index.js" });
...
r = await tools["ultron.run_command"].run({ command: "node -e \"process.exit(3)\"" });
...
try { await tools["ultron.read_file"].run({ file: "../../../etc/passwd" }); assert.fail("should have thrown"); }
catch (e) { assert.match(e.message, /escapes the workspace/); ok("workspace escape is blocked"); }

// 7. git init + commit (real hash)
await tools["ultron.write_file"].run({ file: "package.json", content: JSON.stringify({ name: "ultron-test", version: "1.0.0", scripts: { build: "node -e \"console.log('built')\"", test: "node -e \"console.log('tested')\"" } }, null, 2) });
r = await tools["ultron.git_init"].run({});
assert.equal(r.ok, true); ok("git_init creates a real repo");
await tools["ultron.run_command"].run({ command: "git config user.email t@e.st && git config user.name test" });
r = await tools["ultron.git_commit"].run({ message: "initial commit" });
assert.equal(r.ok, true); assert.match(r.hash || "", /^[0-9a-f]{7,}$/); ok(`git_commit returns a REAL hash (${r.hash})`);

// 8. detect + build + test
r = await tools["ultron.detect_project"].run({});
assert.equal(r.pkg, true); ok(`detect_project reads real package.json (pm=${r.packageManager})`);
r = await tools["ultron.build"].run({});
assert.equal(r.exitCode, 0); assert.match(r.stdout, /built/); ok("build runs the real build script");
r = await tools["ultron.test"].run({});
...
assert.equal(riskOf("ultron.run_command", { command: "rm -rf build" }), "dangerous");
ok("safety classifier tags SAFE/REVIEW/DANGEROUS correctly");

// 10. deploy is honestly "not connected" without creds
r = await tools["ultron.deploy"].run({ provider: "vercel" });
...
console.log(`\n${pass} checks passed. ULTRON's tool layer is real and working.`);
