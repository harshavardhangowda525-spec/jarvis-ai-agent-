...
${ctx.videoAvailable
  ? "- For reels/video, call ev_video (Magic Hour) — text-to-video from a prompt, or image-to-video from an EV image (pass contentImageId or imageUrl). For an Instagram Reel ALWAYS use aspect 'portrait' (9:16). Video rendering takes a few minutes: ev_video returns a projectId, so tell the user it's rendering and call ev_video action 'check' with that projectId when they ask if it's ready (keep checking until it returns a URL). Never claim a video exists until 'check' returns its URL."
...
- To POST a Reel to Instagram (only after approval), pass the finished video URL (the public URL ev_video returned) to ev_instagram action 'publish_reel' with a caption. Reels must be MP4, 9:16, 3–90s. It's async: if Instagram is still processing the video you'll get a containerId — call publish_reel again with that containerId to finish. Only report it published when the API returns a media id.
